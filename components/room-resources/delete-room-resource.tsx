...
export const DeleteRoomResource = ({
  resourceTransfered,
  disclosure,
}: {
  ...
  const handleDeleteResource = async () => {
    ...
    try {
      ...
      await RoomResourceService.delete(resourceTransfered.id.toString());
      mutate(
        (key) =>
          Array.isArray(key) && key[0] === `/room-items/room/${currentRoomId}`
      );
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>
                  Bạn có thật sự muốn thu hồi tài nguyên này không ? Hành động
